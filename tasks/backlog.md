...
## 執筆: 「OSSのバグ報告」記事 【草案あり・未完】

別リポジトリでの作業中に OSS のバグを見つけて upstream へ報告した
（`anthropics/claude-plugins-official` #4907）。その過程を記事にする。

- **草案**: `content/posts/three-same-mistakes-oss-bug-report/index.md`（`draft: true`）
- **引き継ぎ資料**: `tasks/handoff-oss-bug-report-article.md`

草案は3部構成（誤りの記録 / 調査の勘所 / AI と人間）で配置済み。
**残っているのは分割判断・実名の扱い・推敲**で、内容の再取材は不要。

**着手前に決めること**（詳細は引き継ぎ資料）:

1. 1本にするか、2〜3本に分けるか（各部が独立見出しになっており分割は容易）
2. エージェント名・プラグイン名を実名にするか（issue URL を載せるなら実名が前提）
3. issue URL を載せるか（upstream 修正で一部が過去形になる）
4. タイトル確定

**公開制約あり**: 元の調査は業務端末で行ったため、動機の部分に公開できない要素がある。
草案は抽象化済みだが、加筆時は引き継ぎ資料の「公開制約」節を必ず読むこと。
