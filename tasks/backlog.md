...
- **記事・一覧ページで `<h1>` が2つ出ている。** サイトロゴ
  （`layouts/_default/baseof.html:59`）とページタイトルの両方が `h1`。
  実測（`hugo` 後の生成物）: 記事2個 / カテゴリ一覧2個 / トップ1個。
  ロゴ側は**ローカルの `layouts/` が既にテーマを上書きしている**ため、
  submodule を触らずに `<div>` か `<p>` へ降格できる。
  → **決めること**: 全ページに影響する変更のため、降格するか現状維持か。
  降格する場合、テーマ CSS の `.logo` がタグ名に依存していないかを先に確認する。
